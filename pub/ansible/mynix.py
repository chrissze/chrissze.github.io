...
"""
1. A full path nix command might call other short path nix commands, like  ~/.nix-profile/bin/nix-env might call other non full path nix-env commands, so I have to expand PATH variable so that subprocess can locate those triggered commands. $HOME does not work in the PATH variable in this module, I have to use ~.
